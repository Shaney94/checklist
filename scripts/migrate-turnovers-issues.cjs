...
(async()=>{
 const db=database(),sql=fs.readFileSync('migrations/009-turnovers-issues.sql','utf8');
 ...
 console.log('Turnover and issue schema ready.');
})().catch(()=>{console.error('Turnover/issue migration failed. Check database connectivity and migration permissions.');process.exitCode=1;});
