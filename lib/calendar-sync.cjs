...
function input(body){
 ...
 const platform=['Airbnb','Booking.com','Vrbo','TurnCal','Custom iCal'].includes(body.platform)?body.platform:'Custom iCal';
...
async function syncOne(store,owner,id,fetchFeed=fetchCalendar){
 ...
 try{const settings={name:row.display_name,checkIn:String(row.check_in).slice(0,5),checkOut:String(row.check_out).slice(0,5)};const bookings=parse(await fetchFeed(unseal(row.encrypted_url,owner)),settings);await store.finish(row,bookings,null);return {id,ok:true,count:bookings.length};}
...
module.exports={input,parse,validate,syncOne,syncMany};
