...
async function syncOne(store,owner,id,fetchFeed=fetchCalendar){
 ...
 try{const settings={url:unseal(row.encrypted_url,owner),name:row.display_name,checkIn:String(row.check_in).slice(0,5),checkOut:String(row.check_out).slice(0,5)};const bookings=parse(await fetchFeed(settings.url),settings);await store.finish(row,bookings,null);return {id,ok:true,count:bookings.length};}
