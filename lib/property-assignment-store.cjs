...
function createStore(db=database()){
 ...
 async accept(user,id){
  ...
  const result=await db.transaction([
   ...
   db`INSERT INTO turnli_cleaner_codes(user_id,code_hash) VALUES(${user.id},${createHash('sha256').update(randomBytes(32)).digest('hex')}) ON CONFLICT(user_id) DO NOTHING`,
