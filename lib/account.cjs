const {resolveRole}=require('./authorization.cjs');
...
function accountUser(user){
 ...
 return {id:user.userId,email:user.email,canInvite:email(user.email)===OWNER_EMAIL,legacyAccess:!!legacy,workspaceId:legacy?'org:'+process.env.TURNLI_TENANT_ID:'user:'+user.userId,role:resolveRole(user,legacy?process.env.TURNLI_TENANT_ID:undefined)};
}
...
function validOrigin(req) {return [ORIGIN,'https://turnli.vercel.app'].includes(req.headers.origin);}
  ...
  try {
    ...
    if(['send-code','verify-code','password-login','set-password'].includes(body.action)) {
      ...
      if(body.action==='send-code') {
        ...
        result ||= address===OWNER_EMAIL?await sdk.otp.signUpOrIn.email(address):await sdk.otp.signIn.email(address);
