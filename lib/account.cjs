...
function permitted(user){return user?.verifiedEmail===true&&(email(user.email)===OWNER_EMAIL||(!!process.env.TURNLI_TENANT_ID&&user.userTenants?.some(t=>t.tenantId===process.env.TURNLI_TENANT_ID)));}
async function currentUser(req,res,sdk=getClient()) {
  ...
  return {email:result.data.email,canInvite:email(result.data.email)===OWNER_EMAIL};
}
...
async function completeLogin(res,sdk,result) {
  ...
  if(!me.ok||!permitted(me.data)) return res.status(403).json({error:'This account does not have access to this workspace. Please ask your host for an invitation.'});
  setCookies(res,result.data.sessionJwt,result.data.refreshJwt);
  return res.status(200).json({user:{email:me.data.email,canInvite:email(me.data.email)===OWNER_EMAIL}});
}
...
module.exports={createHandler,email,OWNER_EMAIL,currentUser,getClient,privateHeaders,setCookies};
