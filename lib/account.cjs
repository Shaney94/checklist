...
async function currentUser(req,res,sdk){
 ...
 try { await sdk.validateSession(token); }
  ...
  const refresh=cookie(req,REFRESH);if(!refresh) return null;
  try { const renewed=await sdk.refreshSession(refresh);token=renewed.jwt;setCookies(res,token,renewed.refreshJwt); }
  catch {setCookies(res,'','');return null;}
 }
 const result=await sdk.me(token);
...
function createHandler(getSdk=getClient){return async function handler(req,res){
 ...
 try {
  ...
  if(body.action==='send-code'){
   ...
   if(!result.ok) return res.status(result.code===429?429:400).json({error:result.code===429?'Please wait before requesting another code.':'Unable to send a code. Please check your invitation and email address, then try again.'});
  ...
  if(body.action==='verify-code'){
   ...
   const me=await sdk.me(result.data.sessionJwt);
