...
function createHandler(getSdk=getClient) {return async function handler(req,res) {
  ...
  try {
    ...
    if(body.action==='register') {
      ...
      if(!registered.ok)return authFailure(res,registered,'We couldn’t create your account. Check the password requirements. If you already have an account, log in or reset your password.');
