...
function createStore(db=database()){
 ...
 const lock=(user,id,revision)=>db`SELECT j.id FROM turnli_cleaning_jobs j JOIN turnli_dashboard d ON d.owner_id=j.owner_id WHERE j.id=${id} AND j.cleaner_user_id=${user} AND turnli_job_assignment_active(j.property_assignment_id,j.owner_id,j.property_id,j.cleaner_user_id) AND j.state='scheduled' AND j.revision=${revision} AND d.data @> jsonb_build_object('properties',jsonb_build_array(jsonb_build_object('id',j.property_id))) FOR UPDATE OF j`;
 return {
 async view(user,id,host){return (await db`SELECT j.id,j.property_id AS "propertyId",p->>'name' AS "propertyName",j.kind,j.state,j.tasks,j.checked,j.revision,j.submitted_at AS "submittedAt",j.reviewed_at AS "reviewedAt",j.review_note AS "reviewNote",
  ...
  WHERE j.id=${id} AND p->>'id'=j.property_id AND ((${host} AND j.owner_id=${user.workspaceId} AND j.state IN ('awaiting_review','approved','issue_reported')) OR (NOT ${host} AND j.cleaner_user_id=${user.id} AND turnli_job_assignment_active(j.property_assignment_id,j.owner_id,j.property_id,j.cleaner_user_id) AND j.state IN ('scheduled','awaiting_review','approved','issue_reported')))`)[0];},
 async add(user,id,revision,photo){
  ...
  const results=await db.transaction([
   ...
   db`INSERT INTO turnli_cleaning_job_photos(id,job_id,uploaded_by,content_type,byte_size,width,height,sha256,bytes)
    ...
    FROM turnli_cleaning_jobs j JOIN turnli_dashboard d ON d.owner_id=j.owner_id WHERE j.id=${id} AND j.cleaner_user_id=${user} AND turnli_job_assignment_active(j.property_assignment_id,j.owner_id,j.property_id,j.cleaner_user_id) AND j.state='scheduled' AND j.revision=${revision}
 ...
 async remove(user,id,revision,photoId){
  const results=await db.transaction([
   lock(user,id,revision),
   db`WITH removed AS (DELETE FROM turnli_cleaning_job_photos f USING turnli_cleaning_jobs j,turnli_dashboard d WHERE f.id=${photoId} AND f.job_id=j.id AND j.id=${id} AND f.uploaded_by=${user} AND j.cleaner_user_id=${user} AND turnli_job_assignment_active(j.property_assignment_id,j.owner_id,j.property_id,j.cleaner_user_id) AND j.state='scheduled' AND j.revision=${revision} AND d.owner_id=j.owner_id AND d.data @> jsonb_build_object('properties',jsonb_build_array(jsonb_build_object('id',j.property_id))) RETURNING f.id)
 ...
 async submit(user,id,revision){
  const results=await db.transaction([
   ...
   db`UPDATE turnli_cleaning_jobs j SET state='awaiting_review',submitted_at=now(),submitted_by=${user},revision=revision+1,updated_at=now()
    WHERE j.id=${id} AND j.cleaner_user_id=${user} AND turnli_job_assignment_active(j.property_assignment_id,j.owner_id,j.property_id,j.cleaner_user_id) AND j.state='scheduled' AND j.revision=${revision}
 ...
 async photo(user,id,photoId,host){return (await db`SELECT encode(f.bytes,'base64') AS data FROM turnli_cleaning_job_photos f JOIN turnli_cleaning_jobs j ON j.id=f.job_id JOIN turnli_dashboard d ON d.owner_id=j.owner_id
  ...
  AND ((${host} AND j.owner_id=${user.workspaceId} AND j.state IN ('awaiting_review','approved','issue_reported')) OR (NOT ${host} AND j.cleaner_user_id=${user.id} AND turnli_job_assignment_active(j.property_assignment_id,j.owner_id,j.property_id,j.cleaner_user_id) AND f.uploaded_by=${user.id} AND j.state IN ('scheduled','awaiting_review','approved','issue_reported')))`)[0];}
 };
}
