...
function parseCalendar(text,options={}) {
 ...
 for(const event of root.getAllSubcomponents('vevent')){
  ...
  const guestMatch=summary.match(/\b(\d{1,3})\s*(?:guests?|people|persons?)\b/i);
  ...
  const title=summary.replace(/^\([^)]*\)\s*/,'').replace(/^Reserved:\s*/i,'').replace(/\s+\d{1,3}\s*guests?\s*$/i,'').trim()||'Reserved stay';
  bookings.push({id,property,title,summary,guests:guestMatch?Number(guestMatch[1]):null,source,arrival,checkout});
 }
