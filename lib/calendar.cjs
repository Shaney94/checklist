...
const {detectSource}=require('./booking-source.cjs');
...
function parseCalendar(text,options={}) {
 if(typeof text!=='string'||!/^BEGIN:VCALENDAR\r?\n/i.test(text.trim())||!/\r?\nEND:VCALENDAR$/i.test(text.trim()))throw Error('Incomplete calendar');
 const root=new ICAL.Component(ICAL.parse(text));if(root.name!=='vcalendar'||(root.getFirstPropertyValue('method')&&!/^publish$/i.test(String(root.getFirstPropertyValue('method')))))throw Error('Full calendar snapshot required');
 const property=options.property||String(root.getFirstPropertyValue('x-wr-calname')||'Property').slice(0,120),bookings=[],cancelledIds=[];
 for(const event of root.getAllSubcomponents('vevent')){
  ...
  if(String(event.getFirstPropertyValue('status')).toUpperCase()==='CANCELLED'){const uid=String(event.getFirstPropertyValue('uid')||'');if(!uid)throw Error('Missing cancellation UID');cancelledIds.push(createHash('sha256').update(uid).digest('hex'));continue;}
  ...
  const source=detectSource({fields:['x-source','x-platform','x-booking-source'].map(key=>event.getFirstPropertyValue(key)),description,eventURL:event.getFirstPropertyValue('url'),feedURL:options.feedURL,prodid:String(root.getFirstPropertyValue('prodid')||'')});
  bookings.push({id:createHash('sha256').update(id).digest('hex'),property,guests:guestMatch&&Number(guestMatch[1])>0?Number(guestMatch[1]):null,...source,arrival,checkout});
 }
 return {property,timeZone:zone,cancelledIds,bookings:bookings.sort((a,b)=>a.arrival.date.localeCompare(b.arrival.date)),updatedAt:new Date().toISOString()};
}
