...
async function deliverInvitation(address,sdk=getClient()){
 ...
 if(existing.ok&&(email(existing.data?.email)!==address||existing.data.status==='disabled'||accountUser(existing.data).role!=='cleaner'))throw Error('Invitation unavailable');
