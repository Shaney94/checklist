...
function createStore(db=database()){return {
 ...
 async list(user,host){
  ...
  return db`SELECT j.id,j.property_id AS "propertyId",p->>'name' AS "propertyName",j.scheduled_date::text AS date,j.kind,j.state,j.revision,(j.cleaner_user_id IS NOT NULL) AS assigned
   ...
   WHERE p->>'id'=j.property_id AND ((${host} AND j.owner_id=${user.workspaceId}) OR (NOT ${host} AND j.cleaner_user_id=${user.id} AND j.state IN ('scheduled','awaiting_review','approved','issue_reported')))
 ...
 async assign(owner,id,revision,cleaner,code){
  ...
  return (await db`WITH changed AS (UPDATE turnli_cleaning_jobs j SET cleaner_user_id=${cleaner},revision=revision+1,updated_at=now()
 ...
 async assigned(user,id){return (await db`SELECT j.id,j.property_id AS "propertyId",p->>'name' AS "propertyName",j.scheduled_date::text AS date,j.kind,j.state,j.tasks,j.checked,j.revision,CASE WHEN j.state='scheduled' THEN p->>'phone' ELSE NULL END AS "hostPhone",COALESCE(p->'faqs','[]'::jsonb) AS faqs
  ...
  WHERE j.id=${id} AND j.cleaner_user_id=${user} AND j.state IN ('scheduled','awaiting_review','approved','issue_reported') AND p->>'id'=j.property_id`)[0];},
 async guide(user,id){
