...
function createStore(db=database()){return {
 ...
 async rotateCode(user){
  ...
  await db`INSERT INTO turnli_cleaner_codes(user_id,code_hash) VALUES(${user},${hash(code)}) ON CONFLICT(user_id) DO UPDATE SET code_hash=EXCLUDED.code_hash,updated_at=now()`;
  return code;
 },
 ...
 async list(user,host){
  ...
  return db`SELECT j.id,j.property_id AS "propertyId",p->>'name' AS "propertyName",j.scheduled_date::text AS date,j.kind,j.state,j.revision,(j.cleaner_user_id IS NOT NULL) AS assigned
   ...
   WHERE p->>'id'=j.property_id AND ((${host} AND j.owner_id=${user.workspaceId}) OR (NOT ${host} AND j.cleaner_user_id=${user.id} AND j.state='scheduled'))
   ORDER BY j.scheduled_date,j.id`;
 },
 async create(owner,propertyId,date,kind){
  return (await db`INSERT INTO turnli_cleaning_jobs(id,owner_id,property_id,scheduled_date,kind,tasks)
   SELECT ${randomUUID()},d.owner_id,${propertyId},${date}::date,${kind},COALESCE(p->${kind},'[]'::jsonb)
   FROM turnli_dashboard d CROSS JOIN LATERAL jsonb_array_elements(d.data->'properties') p
   WHERE d.owner_id=${owner} AND p->>'id'=${propertyId} RETURNING id`)[0];
 },
 async hostJob(owner,id){return (await db`SELECT j.id,j.state,j.revision FROM turnli_cleaning_jobs j JOIN turnli_dashboard d ON d.owner_id=j.owner_id WHERE j.owner_id=${owner} AND j.id=${id} AND d.data @> jsonb_build_object('properties',jsonb_build_array(jsonb_build_object('id',j.property_id)))`)[0];},
 async assign(owner,id,revision,cleaner,code){
  ...
  return (await db`UPDATE turnli_cleaning_jobs j SET cleaner_user_id=${cleaner},revision=revision+1,updated_at=now()
 ...
 async cancel(owner,id,revision){return (await db`UPDATE turnli_cleaning_jobs SET state='cancelled',cleaner_user_id=NULL,revision=revision+1,updated_at=now() WHERE owner_id=${owner} AND id=${id} AND revision=${revision} AND state='scheduled' RETURNING id`)[0];},
 async assigned(user,id){return (await db`SELECT j.id,j.property_id AS "propertyId",p->>'name' AS "propertyName",j.scheduled_date::text AS date,j.kind,j.tasks,j.checked,j.revision,COALESCE(p->'faqs','[]'::jsonb) AS faqs
  ...
  WHERE j.id=${id} AND j.cleaner_user_id=${user} AND j.state='scheduled' AND p->>'id'=j.property_id`)[0];},
 async guide(user,id){
  ...
  return (await db`SELECT COALESCE(g.revision,0) AS revision,COALESCE(g.guide,'{}'::jsonb) AS guide
   ...
   WHERE j.id=${id} AND j.cleaner_user_id=${user} AND j.state='scheduled'
 ...
 async check(user,id,revision,index,checked){return (await db`UPDATE turnli_cleaning_jobs j SET checked=CASE WHEN ${checked} THEN (SELECT COALESCE(jsonb_agg(DISTINCT n),'[]'::jsonb) FROM jsonb_array_elements(j.checked || jsonb_build_array(${index}::integer)) n) ELSE (SELECT COALESCE(jsonb_agg(n),'[]'::jsonb) FROM jsonb_array_elements(j.checked) n WHERE n<>to_jsonb(${index}::integer)) END,revision=revision+1,updated_at=now()
   WHERE j.id=${id} AND j.cleaner_user_id=${user} AND j.state='scheduled' AND j.revision=${revision} AND ${index}>=0 AND ${index}<jsonb_array_length(j.tasks)
   AND EXISTS(SELECT 1 FROM turnli_dashboard d WHERE d.owner_id=j.owner_id AND d.data @> jsonb_build_object('properties',jsonb_build_array(jsonb_build_object('id',j.property_id)))) RETURNING id`)[0];}
};}
