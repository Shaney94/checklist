...
export default function MyCustomers({ model: m, initiallyAdding = false, onCalendarsChanged }: { model: WorkspaceModel; initiallyAdding?: boolean; onCalendarsChanged: () => void }) {
  ...
  return <div className="my-customers">
    ...
    {adding && <form onSubmit={async e => { e.preventDefault(); if (await m.save({ action: "property", name, phone: "", notes: "" })) { setName(""); setAdding(false); setKind(null); } }}>
      <label className="field">Property name<input autoFocus required maxLength={100} value={name} onChange={e => setName(e.target.value)} aria-describedby="customer-name-help" /></label>
      <p id="customer-name-help" className="calendar-help">Use a name you’ll recognise, such as “Anniesland Flat”.</p>
      <div className="dialog-actions"><button className="primary" disabled={m.busy || !m.ready}>Save customer property</button><button type="button" className="back" onClick={() => setAdding(false)}>Cancel</button></div>
    </form>}
    ...
    {!!properties.length && <>
      ...
      <div className="customer-choices" aria-label="Customer properties">{properties.filter(p => p.name.toLowerCase().includes(search.toLowerCase())).map(p => <button className="back" key={p.id} aria-pressed={p.id === m.selected} onClick={() => { m.setSelected(p.id); setKind(null); }}><span>{p.name}</span></button>)}</div>
      ...
      {property && <section aria-label="Customer property details">
        <h3>{property.name}</h3>
        <details><summary>Edit property name</summary><form key={property.id + property.name} onSubmit={e => { e.preventDefault(); void m.save({ action: "property", id: property.id, name: new FormData(e.currentTarget).get("name"), phone: property.phone, notes: property.notes }); }}><label className="field">Property name<input required name="name" maxLength={100} defaultValue={property.name} /></label><button className="back" disabled={m.busy}>Save property name</button></form></details>
        ...
        <div className="dialog-actions">{(["regular", "deep", "faqs"] as Kind[]).map(k => <button className="back" key={k} onClick={() => setKind(k)}>{labels[k]}</button>)}</div>
        {kind && <WorkspaceTools key={property.id + kind} model={m} kind={kind} onClose={() => setKind(null)} />}
        {manage && <CalendarManagement key={property.id} calendars={linked} open onClose={() => setManage(false)} reload={async removed => { await calendar.reload(removed); onCalendarsChanged(); }} refresh={calendar.refresh} propertyContext={{ id: property.id, name: property.name }} />}
      </section>}
    </>}
  </div>;
}
