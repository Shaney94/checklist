(() => {
 ...
 let loginEmail='',intent='invite',returnFocus;
 ...
 function ready(user){
  ...
  else{show(success,'Welcome to Turnly');success.querySelector('p').textContent='Your account is ready. You can now return to the Cleaning Hub.';}
  document.getElementById('signOutButton').hidden=false;
 }
 async function openAccount(join=false){
  intent=join?'join':'invite';returnFocus=document.activeElement;show(signIn,join?'Join Turnly':'Sign in to invite customers');
  document.getElementById('signInDescription').textContent=join?'Enter the email address where you received your invitation.':'Sign in with your owner email. We’ll send you a code.';
  document.getElementById('signOutButton').hidden=true;
  dialog.showModal();
  try{const data=await request();if(!dialog.open)return;if(data.user){ready(data.user);document.getElementById('signOutButton').hidden=false;}}
 ...
 document.getElementById('changeEmail').addEventListener('click',()=>{verify.reset();show(signIn,intent==='join'?'Join Turnly':'Sign in to invite customers');});
 document.getElementById('signOutButton').addEventListener('click',async()=>{try{await request({action:'logout'});dialog.close();}catch(error){message.textContent=error.message;}});
 ...
 if(new URLSearchParams(location.search).has('join')){
  const url=new URL(location);url.searchParams.delete('join');history.replaceState(history.state,'',url.pathname+url.search+url.hash);openAccount(true);
 }
})();
