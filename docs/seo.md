...
The homepage, three cornerstone acquisition pages and Glasgow local page use App Router metadata and server-rendered HTML. The canonical origin comes from the existing server-owned authentication configuration (`https://turnli.io`), never a request host. The Cleaner acquisition URL retains its trailing slash: `/cleaners/airbnb-cleaning-jobs/`. Other routes retain existing slashless normalisation (except `/`). Links, Open Graph URLs and the sitemap use those same URLs.

`src/features/marketing/metadata.ts` lists the public indexable pages. Extend it only for useful, approved content; a future location page should have visible parent breadcrumbs under Airbnb cleaning. Glasgow is the first local page; it follows the national page’s slashless canonical convention. No matching engine or guaranteed availability has been added. Host onboarding still requires the existing explicit server-managed Host role; registration does not self-assign that role. The Cleaner route uses the existing free registration/workspace flow. No enquiry collection or new marketplace flow is implied by Get started.
...
4. Use URL Inspection → Test live URL for `/`, `/airbnb-cleaning`, `/software/airbnb-cleaning`, `/cleaners/airbnb-cleaning-jobs/` and `/airbnb-cleaning/glasgow`. Confirm crawl access and canonicals before requesting indexing.
...
The Glasgow page uses the supplied £60 Regular / £130 Deep reference prices for approximately 52 m², two bedrooms and one bathroom, with supplies and linen/towel wash and dry included. These are qualified reference prices, not an online quote. Its CTA uses existing registration and explicitly states that property-based quoting and matching are still being developed. Glasgow guidance focuses on the building handover and links to official Scottish licensing information; it makes no neighbourhood coverage claims.
