...
`turnli_cleaning_jobs` belongs to that workspace and a property in its document. Jobs represent cleaning work, distinct from reservations. Manual jobs remain supported; linked reservations now maintain planned turnover jobs as described in [the core Cleaner workflow](cleaner-core-workflow.md). Creating a job snapshots the selected regular/deep task list; later template changes affect future jobs. Checklist progress belongs to the job and persists in PostgreSQL, including through reassignment. Manual jobs can be assigned, reassigned, unassigned or cancelled. Automatic turnover jobs follow the property assignment. Explicit Host cancellation is terminal; reconciliation may restore an automatically cancelled plan if its reservation reappears. Dates do not assert physical checkout or automatically authorize completion.

An authenticated, verified Cleaner receives a random 256-bit assignment code automatically. Its SHA-256 lookup hash and an encrypted copy are persisted in `turnli_cleaner_codes`, keyed by the provider user ID. Migration `012-cleaner-code-display.sql` enables repeat display/copy using the existing content encryption key with user-bound context. Apply with `node scripts/migrate-cleaner-code-display.cjs` using the configured database before deploying. Old hash-only codes require deliberate replacement; opening the workspace never invalidates them. Codes never appear in URLs or browser storage. Rotation invalidates the previous code without changing existing assignments. A code only identifies an account for assignment; it never authenticates a caller. There is no public directory or identity search. The Host's own eligible job is checked before the code lookup. Assignment also verifies the Cleaner’s current enabled, verified, server-managed role through Descope and rechecks the code hash when writing.
