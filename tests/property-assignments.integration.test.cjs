...
test('PostgreSQL: email acceptance, property defaults, isolated calendars, evidence and revocation',{skip:!enabled},async()=>{
 ...
 try{
  ...
  const invitation=await a.invite(host,property,cleaner.email);assert(invitation);assert.deepEqual((await a.onboarding(cleaner)).map(i=>Object.keys(i).sort()),[['id','propertyName']]);assert.equal((await a.onboarding(outsider)).length,0);assert(await a.pending(cleaner,invitation.id));assert.equal(await a.pending(outsider,invitation.id),undefined);assert.equal((await a.inbox(outsider)).length,0);assert.equal(await a.accept(outsider,invitation.id),undefined);assert.equal(await a.guide(cleaner.id,invitation.id),undefined);
  assert.equal(await a.invite(host,property,outsider.email),undefined);
  const [accepted,duringAccept]=await Promise.all([a.accept(cleaner,invitation.id),j.create(owner,property,'2026-09-23','regular')]);assert(accepted);assert.equal(await a.pending(cleaner,invitation.id),undefined);assert(await j.assigned(cleaner.id,duringAccept.id));assert.equal(await a.accept(cleaner,invitation.id),undefined);
  ...
  const [revoked,duringRevoke]=await Promise.all([a.revoke(owner,invitation.id),j.create(owner,property,'2026-09-24','regular')]);assert(revoked);assert.equal((await a.onboarding(cleaner)).length,0);assert.equal(await a.pending(cleaner,invitation.id),undefined);assert.equal(await j.assigned(cleaner.id,duringRevoke.id),undefined);assert.equal((await a.inbox(cleaner)).length,0);assert.equal(await a.guide(cleaner.id,invitation.id),undefined);assert.equal((await a.calendars(cleaner.id,invitation.id)).length,0);
  ...
  const invitation2=await a.invite({...host,workspaceId:other},foreign,cleaner.email);await db`UPDATE turnli_property_assignments SET expires_at=now()-interval '1 hour' WHERE id=${invitation2.id}`;assert.equal(await a.pending(cleaner,invitation2.id),undefined);assert.equal(await a.accept(cleaner,invitation2.id),undefined);
